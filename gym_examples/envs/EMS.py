...
class EnergyManagementEnv(gym.Env):
    def __init__(self):
        ...
        self.range_dict_cont = {"usage": [75000.0*4, 249000.0*4],
                                ...
                                "power_from_generator": [0.0, 60800.0*4]}
        self.range_dict_cont_r = {"usage": [-1, 1], "day_ahead_price": [-1, 1],
                                  "solar_generation": [-1, 1], "soc_battery": [-1, 1],
                                  "power_from_grid": [-1, 1],
                                  "power_from_battery": [-1, 1],
                                  "power_from_generator": [-1, 1]}
        self.terminated = False
        self.truncated = False
        self.prob_obs = ["soc_battery", "power_from_battery", "power_from_generator"]
        ...
        self.power_from_grid_max = self.range_dict['usage'][1] - self.range_dict['solar_generation'][0] - \
                                   self.range_dict['power_from_battery'][0] - self.range_dict['power_from_generator'][0]
        self.power_from_grid_min = self.range_dict['usage'][0] - self.range_dict['solar_generation'][1] - \
                                   self.range_dict['power_from_battery'][1] - self.range_dict['power_from_generator'][1]
        ...
        self.action_space = spaces.Box(low=np.array([self.range_dict_r["power_from_battery"][0],
                                                     self.range_dict_r["power_from_generator"][0]]),
                                       high=np.array([self.range_dict_r["power_from_battery"][1],
                                                      self.range_dict_r["power_from_generator"][1]]),
    ...
    def _get_obs(self):
        return {
            ...
            "power_from_generator": np.array([self._power_from_generator])
        }
    ...
    def get_energy_cost(self):
        ...
        power_from_generator = self.descale_value(self._power_from_generator,
                                                  self.range_dict['power_from_generator'][0],
                                                  self.range_dict['power_from_generator'][1])
        ...
        cost_generator = 0.25 * self.generator_kwh_price * power_from_generator
        cost_grid = 0.25 * day_ahead_price * power_from_grid
        energy_cost = cost_grid + cost_generator
        return energy_cost
    ...
    def reset_power_from_grid(self):
        ...
        power_from_generator = self.descale_value(self._power_from_generator,
                                                  self.range_dict['power_from_generator'][0],
                                                  self.range_dict['power_from_generator'][1])
        power_from_grid = usage - solar_generation - power_from_battery - power_from_generator
    ...
    def test_energy_balance(self):
        ...
        power_from_grid = self.descale_value(self._power_from_grid, self.range_dict['power_from_grid'][0],
                                             self.range_dict['power_from_grid'][1])
        balance = usage - solar_generation - power_from_battery - power_from_generator - power_from_grid
        ...
        if extensive_print:
            ...
            print('power_from_generator:' + str(power_from_generator))
    ...
    def step(self, action):
        ...
        if self._quarter_counter == 35040:
            info = self._get_info()
            ...
            self.update_determined_obs()
            # clip values
            power_from_generator = self.descale_value(action[1], self.range_dict['power_from_generator'][0],
                                                      self.range_dict['power_from_generator'][1])
            power_from_generator = np.clip(power_from_generator, self.range_dict["power_from_generator"][0],
                                           self.range_dict["power_from_generator"][1])
            self._power_from_generator = self.scale_value(power_from_generator,
                                                          self.range_dict['power_from_generator'][0],
                                                          self.range_dict['power_from_generator'][1])
